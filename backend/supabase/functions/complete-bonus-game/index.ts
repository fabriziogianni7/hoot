...
async function checkGoldenQuestionsCorrect(
  ...
  if (goldenQuestionIds.length === 0) {
    return false // No golden questions means no extra bounty
  }

  for (const playerSession of playerSessions) {
    // Only check players with wallet addresses (participants who can receive prizes)
    ...
    for (const questionId of goldenQuestionIds) {
      ...
      if (error || !answer || !answer.is_correct) {
        return false // Player got a golden question wrong
      }
    }
  }
  return true // All players got all golden questions correct
}
...
async function getTreasuryFeeSettings(
  ...
  try {
    const provider = new ethers.JsonRpcProvider(rpcUrl)
    const contract = new ethers.Contract(contractAddress, HOOT_BONUS_QUIZ_MANAGER_ABI, provider)
