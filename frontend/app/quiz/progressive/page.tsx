...
export default function ProgressiveQuizAdmin() {
  ...
  const HOOT_PROGRESSIVE_QUIZ_MANAGER_ADDRESSES = {
  ...
  const contractAddress = HOOT_PROGRESSIVE_QUIZ_MANAGER_ADDRESSES[chain?.id === 8453 ? 8453 : 84532];
  ...
  const handleSaveQuiz = async () => {
    try {
      ...
      const quizData = {
        title: quizTitle,
        description: `Progressive quiz with ${questions.length} questions`,
      ...
      // Create game session (this would need to be updated to handle progressive mode)
  ...
  const handleDeployBounty = async (quizId: string) => {
    ...
    // Create progressive quiz
    const { request: createRequest } = await publicClient?.simulateContract({
      address: contractAddress,
      abi: HOOT_PROGRESSIVE_QUIZ_MANAGER_ABI,
      functionName: 'createQuiz',
      args: [quizId, tokenAddress, bountyAmountWei, BigInt(questions.length)],
    ...
    <div className="min-h-screen bg-black text-white p-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold mb-2">Create Progressive Quiz</h1>
          <p className="text-gray-400">
            Create a quiz where prizes are distributed after each question
          </p>
        </div>
