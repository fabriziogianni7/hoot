...
export default function ResultsPageClient({ roomCode }: ResultsPageClientProps) {
            ...
            if (currentGame) {
              const updatedPlayers = currentGame.players.map((player) => {
                ...
                const backendAnswers = answersData.map((answer) => {
                  ...
                  const isCorrect =
                    answer.questions?.correct_answer_index !== null &&
                  ...
                  return {
                    ...
                    isCorrect, // Use calculated value instead of backend
  ...
  const handleCastResult = async () => {
    ...
    const castText =
      `🎯 Just completed "${quiz.title}" quiz!\n\n` +
  ...
  const handleDistributePrizes = async () => {
    ...
    try {
      ...
      if (invokeError) {
        ...
        throw new Error(
          invokeError.message || "Failed to call complete-game function",
        );
      }
      ...
      <div className="relative z-10 container mx-auto py-8 px-4 flex flex-col items-center">
        ...
        {currentPlayer && !isCreator && (
          <div className="bg-purple-900/30 border border-purple-500 rounded-lg p-4 mb-8 w-full max-w-md">
            ...
            <div className="text-4xl font-bold text-center text-purple-100">
              {currentPlayer.score}
            </div>
        ...
        {quizData &&
          quizData.prize_amount > 0 &&
          (() => {
            const prizeToken = quizData.prize_token;

            const isETH =
              !prizeToken || prizeToken.toLowerCase() === ZERO_ADDRESS.toLowerCase();
                ...
                <div className="mb-4 space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">
                      {quizData.prize_amount.toFixed(2)} {tokenSymbol}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>1st Place (40%):</span>
                    <span>
                      {(quizData.prize_amount * 0.4).toFixed(2)} {tokenSymbol}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>2nd Place (30%):</span>
                    <span>
                      {(quizData.prize_amount * 0.3).toFixed(2)} {tokenSymbol}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>3rd Place (20%):</span>
                    <span>
                      {(quizData.prize_amount * 0.2).toFixed(2)} {tokenSymbol}
                    </span>
                  </div>
                  <div className="flex justify-between text-gray-400">
                    <span>Treasury (10%):</span>
                    <span>
                      {(quizData.prize_amount * 0.1).toFixed(2)} {tokenSymbol}
                    </span>
                  </div>
                </div>
